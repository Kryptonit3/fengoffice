...
function open_flash_chart_object_str( $width, $height, $url, $div_name=null, $use_swfobject=true, $base='' )
{
    //
    // return the HTML as a string
    //
    return _ofc( $width, $height, $url, $div_name, $use_swfobject, $base );
}

function open_flash_chart_object( $width, $height, $url, $div_name=null, $use_swfobject=true, $base='' )
{
    //
    // stream the HTML into the page
    //
    echo _ofc( $width, $height, $url, $div_name, $use_swfobject, $base );
}

function _ofc( $width, $height, $url, $div_name=null, $use_swfobject, $base )
{
    //
    ...
    $obj_id = 'chart';
    if (!$div_name) $div_name = 'flashcontent';
    ...
    $swf_path = "public/assets/flash/";
    if( $use_swfobject )
    {
		// Using library for auto-enabling Flash object on IE, disabled-Javascript proof
		$out[] = '<div id="'. $div_name .'"></div>';
		$out[] = '<script type="text/javascript">';
		$out[] = 'var so = new SWFObject("'. $base . $swf_path . '/open-flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
    ...
    $out[] = '<param name="movie" value="'. $base . $swf_path . 'open-flash-chart.swf?data='. $url .'" />';
    $out[] = '<param name="quality" value="high" />';
    $out[] = '<param name="bgcolor" value="#FFFFFF" />';
    $out[] = '<embed src="'. $base . $swf_path . 'open-flash-chart.swf?data=' . $url .'" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
